...
- Exterior and Flybridge Russian `detail` text is owner-supplied from Google Drive `Claudia-z` / `Описания` (`sunseeker_76_exterior_description.txt`, `sunseeker_76_flybridge_description.txt`). User explicitly requested no editing; Exterior has only the service heading removed from the public UI.
