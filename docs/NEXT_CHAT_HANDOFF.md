...
- Exterior and Flybridge Russian `detail` text is owner-supplied from Google Drive `Claudia-z` / `Описания` (`sunseeker_76_exterior_description.txt`, `sunseeker_76_flybridge_description.txt`). User explicitly requested no editing; Exterior has only the service heading removed from the public UI. Flybridge text is restored from the original `Описания` source and must not be replaced with a shortened equipment-only block.
- Foredeck, Cockpit, and Tender Russian text is owner-supplied from the same Drive folder (`sunseeker-76-foredeck-description-ru.md`, `sunseeker-76-cockpit-description-ru.md`, `williams-420-tender-description-ru.md`). The markdown heading maps to the zone title; the body text is split between `copy` and `detail` without rewriting.
- English localization must follow any owner-supplied Russian copy update before deployment. Localize in natural American English and check semantic parity across `en`/`ru`; do not keep stale short English placeholders when Russian copy has moved on.
