# First Agent Layer — Bell Rock / Claudia Z Controlled Yacht Presentation

This package is intended to be unpacked into the root of a GitHub repository.

Its purpose is to give Codex and any other AI agent a stable first layer of project memory, rules, skills, workflow discipline, and safety boundaries before any code work begins.

## Install

Unzip this package into the repository root.

Expected root structure after unpacking:

```text
AGENTS.md
.agent-start.md
.github/
.agents/
docs/
tasks/
```

## How to use in a new Codex chat

Use a short prompt like this:

```text
Open this repository.
Read AGENTS.md and .agent-start.md.
Use the relevant skills from .agents/skills.
Task: [describe the current task].
First give diagnosis and proposed approach. Do not write code yet.
```

## Project position

This repository is for a premium controlled no-index yacht presentation tool and future private access/messenger layer.

It is not a public charter marketplace.
It is not a public sales site.
It is not an SEO site.
It must not show prices.
It must not use direct public rental/charter/legal-offer language.
It must not be indexed by search engines.
It must protect discretion, owner privacy, and client/broker privacy using lawful privacy-by-design.
